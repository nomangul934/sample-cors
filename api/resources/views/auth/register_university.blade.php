...
<div class="login-wrapper wd-300 wd-xs-400 pd-25 bg-white" style="max-height: 90%; overflow-y: scroll">
    ...
    <form class="form-prevent-multiple-submits recapcha-form" method="POST" action="{{ route('register_university') }}">
        @csrf
        <div class="form-group">
            <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" required autocomplete="name" autofocus placeholder="Enter Contact Name">
            @error('name')
        ...
        <!-- form-group -->
        <div class="form-group">
            <input id="university_name" type="text" class="form-control @error('university_name') is-invalid @enderror" name="university_name" value="{{ old('university_name') }}" required autocomplete="university_name" autofocus placeholder="Enter University Name">
